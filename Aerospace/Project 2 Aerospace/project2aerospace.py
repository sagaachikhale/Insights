...
# Grouping by agency and mission_type, and counting the number of missions
...
# Group by 'agency', 'mission_type', and 'complexity' and count
mission_counts_complex = df5.groupby(['agency', 'mission_type', 'complexity']).size().unstack(fill_value=0)

# Define the desired order of complexity levels
...
# Display the resulting table with the ordered columns
...
filtered_df = df7[
    (df7['agency'] == 'ESA') &
    (df7['complexity'] == 'Medium') &
    (df7['mission_type'] == 'Mars')
]

# Separate Traditional and AI-Augmented
...
# Create a combined table from prebuilt comparison_ESA, comparison_JAXA, comparison_NASA

combined_improvement = pd.DataFrame({
    'Feature': comparison_ESA['Feature'],
    'ESA': comparison_ESA['% Improvement'].round(2),
    'JAXA': comparison_JAXA['% Improvement'].round(2),
    'NASA': comparison_NASA['% Improvement'].round(2)
})
...
# Set the 'Feature' column as the index for easier plotting
...
combined_AI_Augmented = pd.DataFrame({
    'Feature': comparison_ESA['Feature'],
    'ESA': comparison_ESA['AI-Augmented'].round(2),
    'JAXA': comparison_JAXA['AI-Augmented'].round(2),
    'NASA': comparison_NASA['AI-Augmented'].round(2)
})
...
combined_traditional = pd.DataFrame({
    'Feature': comparison_ESA['Feature'],
    'ESA': comparison_ESA['Traditional'].round(2),
    'JAXA': comparison_JAXA['Traditional'].round(2),
    'NASA': comparison_NASA['Traditional'].round(2)
})
...
combined_final = pd.DataFrame({
    'Feature': comparison_ESA['Feature'],
    'ESA_Trad': comparison_ESA['Traditional'].round(2),
    'JAXA_Trad': comparison_JAXA['Traditional'].round(2),
    'NASA_Trad': comparison_NASA['Traditional'].round(2),
    'ESA_%improvement': comparison_ESA['% Improvement'].round(2),
    'JAXA_%improvement': comparison_JAXA['% Improvement'].round(2),
    'NASA_%improvement': comparison_NASA['% Improvement'].round(2)
})
...
filtered_phase_data = merged_df4_df5[
    (merged_df4_df5['agency'] == 'ESA') &
    (merged_df4_df5['complexity'] == 'Medium') &
    (merged_df4_df5['mission_type'] == 'Mars')
]

# Separate Traditional and AI-Augmented methods
...
# Merge Traditional and AI-Augmented side by side on phase
...
# Merge the two tables side-by-side on 'Feature'
combined_ESA_JAXA_NASA = pd.DataFrame({
    'phase': comparison_phase_ESA['phase'],
    'ESA_Trad': comparison_phase_ESA['planning_time_hours_Traditional'],
...
# Merge the two tables side-by-side on 'Feature'
combined_ESA1_JAXA1_NASA1 = pd.DataFrame({
    'phase': comparison_phase_ESA1['phase'],
    'ESA_Trad': comparison_phase_ESA1['contingency_coverage_pct_Traditional'],
    'JAXA_Trad': comparison_phase_JAXA1['contingency_coverage_pct_Traditional'],
    'NASA_Trad': comparison_phase_NASA1['contingency_coverage_pct_Traditional'],
    'ESA_%improvement': comparison_phase_ESA1['contingency_coverage_%Improvement'],
    'JAXA_%improvement': comparison_phase_JAXA1['contingency_coverage_%Improvement'],
    'NASA_%improvement': comparison_phase_NASA1['contingency_coverage_%Improvement']
})
...
# Filter based on the given conditions
filtered_resource_data = merged_df5_df8[
    (merged_df5_df8['agency'] == 'ESA') &
    (merged_df5_df8['complexity'] == 'Medium') &
    (merged_df5_df8['mission_type'] == 'Mars')
]
...
# Merge Traditional and AI-Augmented side by side on 	resource
...
# Merge the two tables side-by-side on 'Feature'
combinedd_ESA_JAXA_NASA = pd.DataFrame({
    'resource': comparison_resource_ESA['resource'],
    'ESA_Trad': comparison_resource_ESA['efficiency_pct_Traditional'],
    'JAXA_Trad': comparison_resource_JAXA['efficiency_pct_Traditional'],
    'NASA_Trad': comparison_resource_NASA['efficiency_pct_Traditional'],
    'ESA_%improvement': comparison_resource_ESA['efficiency_%Improvement'],
    'JAXA_%improvement': comparison_resource_JAXA['efficiency_%Improvement'],
    'NASA_%improvement': comparison_resource_NASA['efficiency_%Improvement']
})
...
# Filter for years 2015-2023 and AI-Augmented only
df7_ai_only = df7[
...
ai_trend_summary = df7_ai_only.groupby('year').agg({
    'planning_time_hours': 'mean',
    'quality_assessment': 'mean',
    'knowledge_transfer_pct': 'mean',
    'contingency_coverage_pct': 'mean',
    'resource_efficiency_pct': 'mean',
...
# First, melt the dataframe to long format for easier plotting
df_melted = pd.melt(
    df10,
    id_vars=['scenario_type', 'complexity'],
    value_vars=[
        'trad_planning_time', 'ai_planning_time',
        'trad_contingency_coverage', 'ai_contingency_coverage',
        'trad_resource_efficiency', 'ai_resource_efficiency'
    ],
    var_name='Metric',
    value_name='Value'
)

# Extract method (Traditional or AI) and Feature from 'Metric' column
...
# Optional: You can do the same split by "complexity" too
for feature in features:
    plt.figure(figsize=(14, 6))
    feature_data = df_melted[df_melted['Feature'] == feature]
...
# 1. Group by 'role' and calculate averages
...
# 2. Find role with highest perceived improvement
...
# 2a. Find role with least perceived improvement
...
# 1. Group by 'agency' and calculate averages
...
# 2. Find agency whose experts feel the biggest improvement
...
# 1. Group by 'experience' and calculate averages
...
# 2. Find which experience group rates AI highest
...
# 3. Find which experience group perceives the biggest improvement
